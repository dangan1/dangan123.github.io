...
if(typeof(STATUSMSG)=='undefined') STATUSMSG=1;
...
function keyDownInit(key)
{
  ...
  if(STATUSMSG && (key==123 || key==119 || key==118)) statusMessage()
}
...
function doKPress(e)
{
  ...
  if(!IE && (e.keyCode>=118&&e.keyCode<=123)) e.preventDefault(); // F7-F12
    ...
    if(IE)
     {
      ...
      oArr[i].attachEvent("onkeypress", doKPress)
     }
    else
     {
